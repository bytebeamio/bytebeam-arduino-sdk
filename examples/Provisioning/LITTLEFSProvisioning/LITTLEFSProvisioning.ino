...
void writeFile(fs::FS &fs, const char *path, const char *message) {
  // spiffs like write file operation
  if(!fs.exists(path)) {
    if (strchr(path, '/')) {
...
void setup() {
...
#if FORMAT_LITTLEFS_IN_BEGINNING
  if (!LittleFS.format()) {
  ...
  writeFile(LittleFS, DEVICE_CONFIG_FILE_NAME, deviceConfigWriteStr);  //  write the device configuration string to littlefs
